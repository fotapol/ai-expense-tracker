...
Adds ``household_id``, ``created_by_user_id``, and ``owner_user_id`` columns
to the ``transactions`` table and backfills existing rows so that:
...
def upgrade() -> None:
    ...
    # Add household_id ---------------------------------------------------
    if "household_id" not in existing_cols:
        op.add_column(
            "transactions",
            sa.Column(
                "household_id",
    ...
    # household_id stays NULL (no default household assignment).
    op.execute(
        sa.text(
            """
            UPDATE transactions
    ...
    if "ix_transactions_household_id" not in existing_indexes:
        op.create_index(
    ...
    if "ix_transactions_household_owner" not in existing_indexes:
        op.create_index(
...
def downgrade() -> None:
    ...
    for col in ["owner_user_id", "created_by_user_id", "household_id"]:
